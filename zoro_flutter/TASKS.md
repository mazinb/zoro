...
- [x] **Phase 1 mailbox backend** — email claim + username-picked address, app deep link, service-role-only tables, Resend inbound PDF retrieval (list attachments API), external pipeline ingest bridge (`/api/mobile/mailbox/ingest`), on-device pickup/ack, and 48h purge. Production: apply `local_part` migration; point Resend at `/api/webhooks/resend/inbound` **or** forward from the external mail webhook to `/ingest` with `MAILBOX_INGEST_SECRET`.
