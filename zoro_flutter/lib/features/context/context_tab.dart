...
class _ContextTabState extends State<ContextTab> {
  ...
  String _money(double v, AppModel model) =>
      formatCurrencyDisplay(v, currency: model.displayCurrency);
    ...
    if (hide) {
      return maskSensitiveNumberString(
        formatCurrencyDisplay(
          a.total,
          currency: currencyCodeForPresetCountry(a.currencyCountry),
        ),
      );
    }
  ...
  String _moneyNativeLiability(LedgerLiabilityRow l, {required bool hide}) {
    if (hide)
      return maskSensitiveNumberString(
        formatCurrencyDisplay(
          l.total,
          currency: currencyCodeForPresetCountry(l.currencyCountry),
        ),
      );
    return formatCurrencyDisplay(
      l.total,
  ...
  Widget build(BuildContext context) {
    ...
    final months = monthsWithData.isEmpty
        ? AppModel.recentMonthKeys()
        : monthsWithData;
        ...
        children: [
          Row(
            children: [
              ...
              TabHeaderActions(
                model: model,
                guideEnabled: model.guideEnabledContext,
          ...
          ...model.assets.map((a) {
            ...
            final valueText = hide
                ? _moneyNativeAsset(a, hide: true)
                : _moneyNativeAsset(a, hide: false);
                ...
                child: InkWell(
                  onTap: () {
                    Navigator.of(context).push<void>(
                      MaterialPageRoute(
                        builder: (ctx) => ContextEditorPage.asset(
                          model: model,
                          assetId: a.id,
                        ),
                      ),
          ...
          ...model.liabilities.map((l) {
            ...
            final valueText = hide
                ? _moneyNativeLiability(l, hide: true)
                : _moneyNativeLiability(l, hide: false);
              ...
              child: Card(
                child: InkWell(
                  onTap: () {
                    Navigator.of(context).push<void>(
                      MaterialPageRoute(
                        builder: (ctx) => ContextEditorPage.liability(
                          model: model,
                          liabilityId: l.id,
                        ),
                      ),
                    ...
                    child: Card(
                      child: ListTile(
                        ...
                        onTap: () {
                          Navigator.of(context).push<void>(
                            MaterialPageRoute(
                              builder: (ctx) => ContextEditorPage.expenseBucket(
                                model: model,
                                bucketKey: k,
                              ),
                            ),
                      ...
                      child: ListTile(
                        leading: Icon(
                          ...
                          color: AppModel.spendVsPredictedColor(
                            actual:
                                (model.monthlyEntryFor(mk)?.monthlySpending ??
                                        0)
                                    .toDouble(),
                        ...
                        title: Text(
                          AppModel.formatMonthKeyLabel(mk),
                          style: const TextStyle(fontWeight: FontWeight.w900),
                        ),
                        subtitle: Text(
                          _hint(
                            model.monthlyEntryFor(mk)?.contextMarkdown ?? '',
                          ),
                          maxLines: 1,
                          overflow: TextOverflow.ellipsis,
                        ),
                        ...
                        onTap: () {
                          Navigator.of(context).push<void>(
                            MaterialPageRoute(
                              builder: (ctx) => ContextEditorPage.month(
                                model: model,
                                monthKey: mk,
                              ),
                            ),
