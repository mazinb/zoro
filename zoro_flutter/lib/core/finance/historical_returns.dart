...
enum HistoricalAssetClass {
  ...
  static HistoricalAssetClass fromApi(String? raw) =>
...
List<HistoricalReturnYear> _years(Map<int, double> data) =>
    data.entries
        .map((e) => HistoricalReturnYear(year: e.key, returnPct: e.value))
        .toList()
...
List<HistoricalReturnSeries> defaultHistoricalReturnSeries() => [
  ...
  HistoricalReturnSeries(
    ...
    builtin: true,
    notes:
        'Bloomberg US Aggregate Bond Index total return — optional; FD series is usually a better cash leg.',
    returnsByYear: _years(const {
      1995: 18.46,
      1996: 3.93,
      1997: 9.69,
...
HistoricalReturnSeries? decodeHistoricalReturnSeries(Object? raw) {
  ...
  for (final row in yearsRaw) {
    ...
    years.add(
      HistoricalReturnYear(year: year.round(), returnPct: ret.toDouble()),
    );
  }
...
void decodeCorpusBacktestPrefs(
  ...
  if (ds != null && ds.isNotEmpty) {
    onDebtSeriesId(
      ds == kDefaultUsAggBondSeriesId ? kDefaultCashFdSeriesId : ds,
    );
  }
