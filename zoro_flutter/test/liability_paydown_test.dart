...
void main() {
  ...
  test(
    'setLiabilityPaydownMonthly shifts from invest when over savings slice',
    () {
      final m = AppModel();
      ...
      expect(
        m.liabilities.firstWhere((l) => l.id == id).paydownMonthly,
        closeTo(250, 1),
      );
      expect(m.allocSavingsMonthly, closeTo(250, 1));
      expect(m.allocInvestmentsMonthly, closeTo(750, 1));
      expect(
        m.allocInvestmentsMonthly + m.allocSavingsMonthly,
        closeTo(1000, 1),
      );
    },
  );
