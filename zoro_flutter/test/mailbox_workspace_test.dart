...
void main() {
  ...
  test('parses mailbox claim deep links', () {
    expect(isMailboxClaimUri(Uri.parse('zoro://mailbox/claim?nonce=abc')), isTrue);
    expect(isMailboxClaimUri(Uri.parse('https://www.getzoro.com/mailbox/claim?nonce=abc')), isTrue);
    expect(isMailboxClaimUri(Uri.parse('https://www.getzoro.com/legal')), isFalse);
    expect(mailboxNonceFromUri(Uri.parse('zoro://mailbox/claim?nonce=abc')), 'abc');
  });

  test('seeds allowlist, skips duplicate remote ids, and clears credentials on revoke', () async {
    ...
    await workspace.applyClaim(
      const MailboxClaimInfo(
        address: 'zoro-abc@inbox.getzoro.com',
        mailboxToken: 'zmb_token',
        claimedEmail: 'ada@example.com',
      ),
    );
    expect(workspace.identity.mailboxAddress, 'zoro-abc@inbox.getzoro.com');
    expect(workspace.identity.allowlist, contains('ada@example.com'));

    mailbox.pendingItems = [
      const MailboxPendingItem(id: 'm1', fileName: 'a.pdf', from: 'ada@example.com'),
      const MailboxPendingItem(id: 'm1', fileName: 'a.pdf', from: 'ada@example.com'),
    ];
...
class _FakeMailbox extends MailboxClient {
  _FakeMailbox() : super(httpClient: MockClient((_) async => http.Response('{}', 404)));
  ...
  @override
  Future<List<MailboxPendingItem>> pending({required String mailboxToken}) async => pendingItems;
