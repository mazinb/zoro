...
void main() {
  ...
  test(
    'autoAllocateGoalSavingsWeights favors larger monthly gaps on targets only',
    () {
      final m = AppModel();
      ...
      expect(
        m.savingsMonthlyForGoal(retirement),
        m.investMonthlyForRetirement(),
      );
  ...
  test(
    'target pool assignment respects sort order without double counting',
    () {
      final m = AppModel();
      ...
      for (final a in m.assets) {
        if (a.type == LedgerAssetType.savings && a.id != savings.id)
          a.total = 0;
