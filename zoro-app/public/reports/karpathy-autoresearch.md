...
- **Single GPU** (tested on H100, with community forks for Mac, Windows, and AMD).
...
Swapped the default AdamW betas (0.9, 0.999) to (0.95, 0.98). val_bpb dropped to 0.9901. Small change, tangible improvement. This is the kind of hyperparameter tweak that ML practitioners know — the betas control the momentum and second-moment decay rates in the optimizer, and the default values aren't necessarily optimal for every training regime — but the agent discovered this empirically without ever reading the AdamW paper or any optimizer textbook.
...
The agent replaced standard absolute positional embeddings with rotary (RoPE) embeddings. val_bpb dropped from 0.9901 to 0.9856. A solid move that matches what larger models use in production. RoPE has become standard in modern LLMs (LLaMA, Mistral, Qwen) precisely because it handles extrapolation to longer sequences better than learned sinusoidal or learned position tables — but the agent found this by trial and error on a training script, not by reading the original paper.
...
The original code requires a single NVIDIA GPU (tested on H100), Python 3.10+, and `uv`. Community forks have extended it to macOS (via MLX), Windows with RTX, and even AMD GPUs — though frankly, the community versions are a mixed bag. Some are thorough rewrites; others look like they were made in an afternoon just to claim they exist.

For smaller compute like a MacBook Pro, Karpathy recommends lower-entropy datasets (TinyStories), reduced vocab_size (4096 down to 256 byte-level), lower MAX_SEQ_LEN (down to 256), reduced DEPTH (8 down to 4), and simpler window patterns.
...
## The Ratchet Beyond ML

The same pattern works beyond ML training. Prompt engineers could iterate on system prompts and evaluate against benchmark scores - automating what is otherwise a tedious manual process. Product managers testing landing page variants against conversion rates would find the ratchet approach liberating. And for software debugging? Any domain where changes can be enumerated, applied, and measured programmatically is fair game. The constraint isn't the pattern — it's whether you can write an evaluator that tells you what "good" looks like.
...
The agent tried something, measured the result, kept what worked, discarded what didn't.

That ratchet, always moving forward, never regressing, is what makes this work. Not the individual experiments. Not the specific changes. The discipline of only keeping improvements.
